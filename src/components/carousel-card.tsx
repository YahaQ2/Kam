...
export function CarouselCard({ to, from, message, spotifyId }: CarouselCardProps) {
  ...
  useEffect(() => {
    if (spotifyId) {
      const accessToken = 'YOUR_SPOTIFY_ACCESS_TOKEN'; // Replace with your actual access token
      fetchTrackDetails(spotifyId, accessToken)
          ...
          <div className="mt-4 border-t pt-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium truncate">{trackDetails.songTitle}</p>
                <p className="text-sm text-gray-600 truncate">{trackDetails.artist}</p>
              </div>
