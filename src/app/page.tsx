...
export default function HomePage() {
  ...
  const [activeLatestIndex, setActiveLatestIndex] = useState(0); // Track which of the latest messages to show
  ...
  // Setup carousel rotation for both main slides and latest message rotations
  useEffect(() => {
    if (loading || latestMessages.length === 0) return;
    
    const interval = setInterval(() => {
      if (activeSlide === 0) {
        // If on first slide, rotate through latest messages
        setActiveLatestIndex(prev => (prev + 1) % latestMessages.length);
      } else {
  ...
  // Component for individual message cards with fixed message border proportions
  const MessageCard = ({ message, featured = false }: { message: Menfess, featured?: boolean }) => (
    <Link href={`/message/${message.id}`} className="block h-full">
      ...
      >
        ...
        {/* Modified CarouselCard with properly proportioned message border */}
        <div className="px-4 py-2">
          <div className="bg-gray-700 rounded-xl overflow-hidden">
            ...
            {message.track?.title && (
              <div className="flex items-center border-t border-gray-600 p-3 bg-gray-750">
