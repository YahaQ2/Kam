...
useEffect(() => {
  const fetchMessages = async () => {
    setLoading(true);
    setError(null); // Reset error state sebelum pengambilan data
    try {
      const response = await fetch(`https://unand.vercel.app/v1/api/menfess-spotify-search`);
      if (!response.ok) {
        throw new Error("Failed to fetch messages."); // Lempar error jika respons tidak oke
      }
      ...
      if (responseData.status && Array.isArray(responseData.data)) {
        ...
        throw new Error("Invalid data format."); // Lempar error jika format data tidak valid
      }
    } catch (err) {
      if (err instanceof Error) {
        console.error("Error fetching messages:", err); // Mencatat kesalahan ke konsol
        setError(err.message); // Set pesan kesalahan untuk ditampilkan di UI
      } else {
        console.error("An unknown error occurred:", err); // Mencatat kesalahan yang tidak diketahui
        setError("An unknown error occurred."); // Set pesan kesalahan umum
      }
    } finally {
  ...
  fetchMessages(); // Memanggil fungsi untuk mengambil data
}, []);